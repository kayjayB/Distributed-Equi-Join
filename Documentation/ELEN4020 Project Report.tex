...
The OpenMP library is a multiprocessing application program inference (API) used to develop shared-memory parallel programs\cite{PerformComp}. The library allows the same code base to be run identically across a range of different operating systems. It takes the form of a set of compiler directives (pragmas) which are used for thread creation, synchronization of operations and allow the management of shared memory when implementing pthreads. 
...
MPI can be used for a variety of parallel computing models. These models can either perform the parallelization of data where the same instructions are carried out simultaneously on multiple data items (SIMD) or task parallelization where different instructions are performed on separate sections of data (MIMD)\cite{PerformComp}. The interface is able to provide a powerful, efficient method of executing parallel programs in a portable manner. This means that the program can be run on a variety of different computers or devices with a variety of operating systems and protocols.

\subsection{MapReduce}

MapReduce is a functional language-inspired parallel programing paradigm for large-scale processing frameworks\cite{comparingMPIMapReduce}. The model has been adopted across a number of platforms due to it's simplicity and suitability in solving to practical problems. The algorithm consists of two distinct parts, namely the mapper function and reducer function. 

The mapper function takes as an input a key and a value and emits a set of intermediate key-value pairs. A number of map processes can be implemented according to the needs of the programmer in order to partition and process input data. These processes exist independently of one another meaning no collaboration is required when executed in parallel. The resultant process groups together all values associated with a given key in order to generate key-value pairs. These pairs are then passed to the reducer function accordingly.

The reducer function then makes use of the output from the mapper function. It takes as inputs a key and a list of values associated with the key. The reduce process is the responsible for aggregating values associated with the same key in order to produce a data set which is inherently smaller than the original. 

The MapReduce model is well suited for data intensive processing for a number of reasons. Firstly, the model is able to sequentially work through each record one at a time set without needing to have the entire data set loaded into memory. The program is also conveniet to use as the MapReduce model performs the data partitioning as well as process creation and synchronization\cite{PerformComp}. This means that the distributed and parallel processing is all performed behind the scene without needing input from the programmer.

The Phoenix project allows the implementation of the Mapreduce paradigm on a platform with multiple cores.
